...
import type { FitAnnotatedModel } from "../../api/types.js";
...
export function RecommendedTab({ onDownloadStarted }: { onDownloadStarted: () => void }) {
    ...
    <div className="il-model-tiers">
      {grouped.map(({ tier, models }) => (
        <section key={tier} className="il-model-tier">
          ...
          <div className="il-model-grid">
            {models.map((m) => (
              <RecommendedCard key={m.id} model={m} onDownloadStarted={onDownloadStarted} />
            ))}
          </div>
        </section>
      ))}
  ...
  return (
    <div className="il-model-card">
      ...
      <div className="il-model-card__foot">
        ...
        <Button size="sm" variant={model.fits ? "primary" : "secondary"} onClick={download} disabled={busy}>
