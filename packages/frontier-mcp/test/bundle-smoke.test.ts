...
describe.skipIf(!bundleExists)("dist/interloom-mcp.js stdio smoke test", () => {
  it(
    "speaks MCP initialize over stdio and lists exactly the 7 pinned tools",
    async () => {
      ...
      try {
        ...
        const initializeResponse = messages.find((m) => m.id === 1);
        expect(initializeResponse?.result).toMatchObject({ serverInfo: { name: "interloom" } });

        const toolsListResponse = messages.find((m) => m.id === 2);
        const tools = (toolsListResponse?.result as { tools: Array<{ name: string }> } | undefined)?.tools ?? [];
